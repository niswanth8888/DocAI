...
import { getAdminSearches } from '../api/client';
import { SearchHistoryItem } from '../types/api';
import { Search, Calendar, FileText, ChevronDown, ChevronUp, AlertCircle, Compass, Users, Download } from 'lucide-react';
...
const AdminSearchAnalytics: React.FC = () => {
  ...
  const fetchSearches = async () => {
    ...
    try {
      ...
      const data = await getAdminSearches(params);
      setSearches(data.searches);
      setCount(data.count);
    } catch (err: any) {
      console.error(err);
      setError('Could not retrieve global search history.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full mx-auto space-y-6 text-slate-300 font-sans p-6">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
          ...
          <span>Organization Search Audit Logs</span>
        </h1>
        <p className="text-xs text-slate-400">
          Monitor search patterns, query success levels, and reference citations across all departments.
        </p>
      </div>
      ...
      <form onSubmit={handleSearchSubmit} className="bg-[#0d0d0d] border border-[#2A2A2A] rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-stretch md:items-center text-xs">
        <div className="flex-1 relative">
          ...
          <input
            ...
            placeholder="Search queries, keywords, or employee names..."
      ...
      {loading ? (
        <div className="text-center py-12 text-slate-500 text-xs">
          Loading global searches database...
        </div>
      ) : error ? (
              ...
              >
                ...
                {isExpanded && item.sources && item.sources.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-[#2A2A2A]/60 space-y-3.5 text-xs">
                    <h4 className="font-bold text-white text-[11px] tracking-wider uppercase text-slate-400">
